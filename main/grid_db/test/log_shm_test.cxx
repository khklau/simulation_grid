...
{
    static const char SLAVE_NAME[] = "log_shm_service";
...
TEST(log_shm_test, startup_and_shutdown_benchmark)
{
...
TEST(log_shm_test, forward_iterate)
{
    config conf(ipc::shm, bfs::unique_path().string());
    service_launcher launcher(conf);
    service_client client(conf);
...
TEST(log_shm_test, backward_iterate)
{
    config conf(ipc::shm, bfs::unique_path().string());
    service_launcher launcher(conf);
    service_client client(conf);
...
TEST(log_shm_test, out_of_bounds)
{
    config conf(ipc::shm, bfs::unique_path().string());
    service_launcher launcher(conf);
    service_client client(conf);
