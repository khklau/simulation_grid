...
{
    bi::managed_shared_memory shmem(bi::create_only, "shmem_front_reference_lifetime", 4096);
    sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type* buf = shmem.construct<sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type>("RB")(4, &shmem);
    const boost::uint32_t expected1 = 1U;
    buf->push_front(expected1);
    const boost::uint32_t& actual1 = buf->front();
    EXPECT_EQ(expected1, actual1) << "front element is not just pushed element";
    ...
    EXPECT_EQ(2U, buf->element_count()) << "element count is wrong";
    ASSERT_EQ(expected1, actual1) << "reference to original front element is lost after subsequent push";
    bi::shared_memory_object::remove("shmem_front_reference_lifetime");
}

TEST(multi_reader_ring_buffer_test, shmem_already_popped_back_element)
{
    bi::managed_shared_memory shmem(bi::create_only, "shmem_already_popped_back_element", 4096);
    sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type* buf = shmem.construct<sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type>("RB")(4, &shmem);
    const boost::uint32_t expected1 = 1U;
    buf->push_front(expected1);
    const boost::uint32_t& actual1 = buf->back();
    ...
    buf->pop_back(expected3);
    ASSERT_EQ(1U, buf->element_count()) << "attempting to pop an element that was already popped should have failed";
    bi::shared_memory_object::remove("shmem_already_popped_back_element");
}

TEST(multi_reader_ring_buffer_test, shmem_overfill)
{
    bi::managed_shared_memory shmem(bi::create_only, "shmem_overfill", 4096);
    sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type* buf = shmem.construct<sg::shmem_multi_reader_ring_buffer<boost::uint32_t>::type>("RB")(4, &shmem);
    EXPECT_TRUE(buf->empty()) << "initial state of ring buffer is not empty";
    buf->push_front(1U);
    buf->push_front(2U);
    ...
    const boost::uint32_t& actual1 = buf->back();
    ASSERT_NE(expected1, actual1) << "back element was not popped";
    bi::shared_memory_object::remove("shmem_overfill");
}
